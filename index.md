...
- 게임을 단순화하면서도 격투 게임 특유의 심리전 싸움의 재미를 만들기 위해 다른 게임들을 참고했다.
...
초보자를 위한 튜토리얼 부재, 어려운 격투 게임 용어 사용, 온라인 환경, 밸런스 문제 등 최근의 많은 격투 게임들이 가진 문제점들을 극복하고자 만든 게임이다.

극히 단순한 조작법과 룰을 적용한 격투 게임이다. 일정 횟수 이상 가드를 할 수 없어서 상대의 공격을 피하거나 상대 움직임의 빈틈을 이용하는 플레이가 주가 된다.
...
이때 "아무 행동도 하지 않아야만 가드가 된다"는 점과 "시간 종료시 마지막에 공격한 플레이어가 이긴다"라는 룰이 있다.
위 룰을 악용하려고 "아무 행동도 하지 않고 자동 가드만 하는 플레이"를 하는 유저를 억제하기 위해서
"10초간 조작을 하지 않은 플레이어의 체력을 1칸씩 감소 시킨다."라는 룰을 적용한다.

- UI와 게임 메커니즘
양 플레이어 화면 하단부에 스페셜 게이지가 있다. S키에 대응하는 버튼으로 기술 사용 가능. (캐릭터마다 성능이 다르고 게이지의 디자인도 다르다)
한 라운드가 아닌 한 매치(전체 경기) 동안 단 3개가 유지되므로 상당히 신중해서 S 기술을 사용해야 한다. S 기술의 성능은 게임의 역전을 이뤄낼 수도 있을 정도의 강력한 판정을 가졌다.
S 게이지는 아이콘으로 표시되며 사용 시마다 하나씩 줄어든다.
...
하지만 위에서 설명 했듯이 자동 가드에 의존하면 게임을 이기
