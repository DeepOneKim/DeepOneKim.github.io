...
- 마치 가위바위보와 같이 서로 어떤 행동을 할지 예측해가며 플레이하고, 상대의 행동을 잘 캐치한 플레이어가 승기를 잡음
- 상대의 심리를 읽기 위해 서로 신중한 공격을 펼쳐 상대를 이겨야 한다. 공격을 마구 누르면 좋지 않을 것
...
- 체력바, 필살기 게이지등의 표시를 단순화
- 직관적인 게임 디자인으로 누구나 게임의 현재 상황을 알 수 있음
...
방향키와 점프 버튼, 다이브킥 버튼만 존재하는 극단적으로 단순한 조작이 특징. 다이브킥에 맞으면 반드시 한방에 패배하므로 체력 게이지가 없다.
...
이 게임은 스트리트 파이터 시리즈의 패러디 게임으로, 현대 2D 격투게임들이 갖고 있는 여러가지 문제점을 해소해 보자는 아이디어로 만들어 졌다.

초보자를 위한 튜토리얼이 없거나, 어려운 격투 게임 용어, 온라인 환경, 밸런스 문제 등 최근의 많은 격투 게임들이 가진 문제점들을 극복하고자

극히 단순한 조작법과 룰을 적용한 격투 게임이다. 일정 횟수 이상 가드를 할 수 없어서 상대의 공격을 피하거나 상대 움직임의 빈틈을 이용하게 된다.
